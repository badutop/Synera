...
$post_obj  = $args['post'];
$category  = get_post_meta($post_obj->ID, 'category', true);
...
<a href="<?php echo esc_url($permalink); ?>" class="reveal group block overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-card transition hover:-translate-y-1 dark:border-white/10 dark:bg-white/[0.03]" style="transition-delay:<?php echo esc_attr($index * 0.1); ?>s">
	...
	<div class="p-6">
		<?php if ($category) : ?>
			<span class="inline-flex rounded-full bg-primary-50 px-3 py-1 text-xs font-semibold uppercase text-primary dark:bg-primary-500/10"><?php echo esc_html($category); ?></span>
		<?php endif; ?>
